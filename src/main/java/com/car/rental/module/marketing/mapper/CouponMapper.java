...
public interface CouponMapper extends BaseMapper<Coupon> {
     ...
     * 查询券关联的车辆ID列表
     */
    @Select("SELECT car_id FROM coupon_car WHERE coupon_id = #{couponId}")
    List<Long> selectCarIdsByCouponId(@Param("couponId") Long couponId);

    /**
     * 查询券关联的车辆名称列表（用于列表展示）
     */
    @Select("SELECT ci.name FROM coupon_car cc " +
    ...
    @org.apache.ibatis.annotations.Update(
        "UPDATE coupon SET received_count = received_count + 1, version = version + 1 " +
     ...
     */
    @org.apache.ibatis.annotations.Update(
        "UPDATE coupon SET received_count = GREATEST(received_count - 1, 0), version = version + 1 " +
        "WHERE id = #{couponId}"
    )
     ...
     */
    @org.apache.ibatis.annotations.Update(
        "UPDATE coupon SET used_count = GREATEST(used_count - 1, 0), version = version + 1 " +
        "WHERE id = #{couponId}"
    )
